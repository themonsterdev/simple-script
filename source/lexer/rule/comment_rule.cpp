...
bool FCommentRule::Match(const std::string& text, size_t& index, FToken& token) const
{
    ...
    if (text[index] == '#' || text.substr(index, 2) == "//")
    {
        ...
        token.SetType(eTokenType::Comment);
        token.SetLexeme(text.substr(startPos, index - startPos));
