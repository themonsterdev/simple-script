...
bool FBooleanRule::Match(const std::string& text, size_t& index, FToken& token) const
{
    ...
    if (text[index] == 't' || text[index] == 'f')
    {
        ...
        if (text.substr(index, 4) == "true")
        {
            ...
            token.SetType(eTokenType::Boolean);
            token.SetLexeme("true");
