...
section .rodata                  ; Section pour les données en lecture seule
    ...
    stdout_query equ -11         ; Valeur pour obtenir le handle de la sortie standard (STD_OUTPUT_HANDLE)
    status equ 0                 ; Code de sortie du programme

section .data                  ; Section pour les données variables
    stdout dq 0                ; Réserve de l'espace pour le handle de la sortie standard (8 octets pour 64 bits)
...
start:
    ...
    mov rcx, stdout_query      ; Place la valeur -11 dans le registre rcx
    call GetStdHandle          ; Appelle la fonction GetStdHandle pour obtenir le handle de la sortie standard
    mov [rel stdout], rax      ; Stocke le handle de la sortie standard dans la variable stdout

    ; Écrire la chaîne de caractères sur la sortie standard
    mov  rcx, [rel stdout]     ; Place le handle de la sortie standard dans le registre rcx
